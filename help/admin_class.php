...
Class Action {
	...
	function save_specialist(){
		...
		$check = $this->db->query("SELECT * FROM specialists where email ='$email' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO specialists set $data");
		}else{
			$save = $this->db->query("UPDATE specialists set $data where id = $id");
		}
	...
	function delete_specialist(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM specialists where id = ".$id);
		if($delete){
			return 1;
		}
	}
